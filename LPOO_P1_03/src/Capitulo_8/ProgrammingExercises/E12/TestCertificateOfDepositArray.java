...
public class TestCertificateOfDepositArray {
	...
	public static void main(String[] args) {
		...
		for (int j = 0; j < certArray.length; j++) {
			System.out.println(
					"informacion del certificado: " + (j + 1) + "\n\tNumero:  " + certArray[j].getCertificateNumber()
							+ "\n\t apellido del titular: " + certArray[j].getHoldersLastName() + "\n\tBalance: "
							+ certArray[j].getBalance() + "\n\tfecha de ingreso: " + certArray[j].getIssueDate()
							+ "\n\tfecha de vencimiento: " + certArray[j].getMaturityDate());
		}
	}
}
